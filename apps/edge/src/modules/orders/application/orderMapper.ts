...
export function mapOrderToResponse(order: Order): OrderResponse {
  ...
  return {
    ...
    items: order.items.map((item) => ({
      ...
      lineBase: item.getLineBase().toJSON(),
      lineTax: item.getLineTax().toJSON(),
      ...
      productSnapshot: {
        ...
        taxPolicyVersion: item.snapshot.taxPolicyVersion,
        taxProfileId: item.snapshot.taxProfileId?.toString() ?? null,
        taxProfileRevision: item.snapshot.taxProfileRevision,
    ...
    taxTotal: order.getTaxTotal().toJSON(),
    total: order.getTotal().toJSON(),
