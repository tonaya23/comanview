...
import type { AuthenticatedActor } from '../../app/authContext.js';import type { TaxAdministrationCommand } from '@comanview/contracts';import { AppError } from '../../app/errorHandler.js';
export class TaxAdministrationService{constructor(private repository:TaxAdministrationRepository,private binding:{tenantId:string;locationId:string;edgeId:string}){}
  ...
  execute(command:TaxAdministrationCommand,actor:AuthenticatedActor){if(!actor.permissions.includes('TAX_PROFILE_MANAGE'))throw new AppError('PERMISSION_DENIED',403,'Tax administration permission required.');
    ...
    try{return this.repository.execute(command,this.binding,audit);}catch(error){const code=error instanceof Error?error.message:'';if(/^(ADMINISTRATION_|TAX_|PRODUCT_|COMMAND_)/.test(code))throw new AppError(code,409,'La configuración fiscal no fue modificada.');throw error;}}
}
