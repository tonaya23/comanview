...
export function deviceRoutes(service:DeviceService,guard:AuthGuard):FastifyPluginAsyncZod{return async(app)=>{
  ...
  app.post('/devices/:deviceId/revoke',{preHandler:guard.requirePermission(PERMISSIONS.DEVICE_REVOKE),schema:{params:DeviceParams,body:RevokeDeviceRequestSchema,response:{200:z.object({revoked:z.literal(true)})}}},async(req)=>await service.revoke(req.params.deviceId,req.body.reason,req.body.commandId,actorFrom(req)));
  app.get('/installation/readiness',{preHandler:guard.requirePermission(PERMISSIONS.INSTALLATION_READINESS_VIEW),schema:{response:{200:InstallationReadinessSchema}}},async()=>await service.readiness());
};}
